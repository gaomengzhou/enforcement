...
class app extends Component {
  ...
  onSelects = value => {
    this.setState({ partiesTypes: value });
  };
  ...
  render() {
          ...
          <Col span={this.state.info ? (this.state.info.status >= 5 && this.state.info.dispatchVos[0].executor) ? 5 : 5 : 5}>
            ...
            >
              {getFieldDecorator(`assistTime[${index}]`, {
                ...
                addonAfter={<SelectAfter index={index} />}
              // disabled={(info.dispatchVos && info.dispatchVos.length > 0 && info.dispatchVos[0].isManage == 0 || userRole != 3) && ((flag != 1 && flag != 2) || isManageFlag[index])}
              />)}
            </Form.Item>
          </Col>
    ...
    function setDataBtn(e) {
      ...
      that.props.form.validateFields((error, values) => {
        if (!error) {
          const arrlist = [];
          for (const i of values.isManage) {
            if (i == true) {
              arrlist.push("1");
            } else {
              arrlist.push("0");
          ...
          that.props.dispatch({
            type: 'compmodels/setModalData',
            payload: {
              workOrderId,
              dispatchVos: {
                assistDepartment: values.assistDepartment[0], // 部门
                assistTime: arrlist != [] ? values.assistTime[0] : null, // 时限
                assistWorker: values.assistWorker[0], // 执行员
                isManage: arrlist  // 单位
              },
            }
          })
              ...
              <Row>
                  ...
                  <div id="tableListForm" style={{ padding: '0 10px 0 0' }}>
                    ...
                    <Row style={{ marginLeft: 0 }} gutter={{ xl: 8, xxl: 40 }}>
                        ...
                        <Col span={12}>
                          ...
                          >
                            {getFieldDecorator('ownCommunity', {
                              ...
                              <Select onSelect={this.communityGrid} placeholder="请选择" disabled={flag != 1 && flag != 2}>
                                {/* {ownCommunityDesc ? ownCommunityDesc.map(item => {
                                  console.log(item)
                                  return (
                        ...
                        <Col span={12}>
                          <FormItem label="所属网格" labelCol={{ span: 7 }} wrapperCol={{ span: 15 }}>
                            {getFieldDecorator('ownGridNumber', {
                              initialValue: ownGridNumberDesc || '',
                              ...
                              >
                                {/* {newgridList.length > 0 ?
                                  newgridList.map(
                                    ...
                                    item => <Option value={item.id}>{ownGridNumberDesc}</Option>
                                  )
                                } */}
                                <Option value={1}>{ownGridNumberDesc}</Option>
                              </Select>
                            )}
                          </FormItem>
                        </Col>
                      ...
                      <Col span={24}>
                        <Col span={12}>
                          <FormItem
                            ...
                            wrapperCol={{ span: 12 }}
                          >
                            {getFieldDecorator('partiesTypes', {
                              initialValue: partiesType || '1',
                              rules: [
                                {
                                  required: true,
                                  message: '请选择案件小类',
                      ...
                      <Col span={24}>
                        <Col span={12}>
                          <FormItem
                            label={partiesTypes == 1 ? '当事人姓名' : '涉事企业名'}
                            labelCol={{ span: 12 }}
                            wrapperCol={{ span: 12 }}
                          >
                            {getFieldDecorator(
                              `${partiesTypes == 1 ? 'partiesUsername' : 'enterprisesName'}`,
                              {
                                initialValue: partiesUsername || '',
                        ...
                        <Col span={12}>
                          <FormItem
                            label={partiesTypes == 1 ? '证件号码' : '机构代码'}
                            labelCol={{ span: 9 }}
                            wrapperCol={{ span: 15 }}
                          >
                            {getFieldDecorator(
                              `${partiesTypes == 1 ? 'identificationNum' : 'institutionalCode'}`,
                              {
                                initialValue: identificationNum || '',
                                // initialValue: info.identificationNum || info.institutionalCode || '',
                                rules: [
                                  {
                                    required: true,
                                    message: partiesTypes == 1 ? '请填写证件号码!' : '请填写机构代码!',
                                  },
                                  {
                      ...
                      <Col span={24}>
                        <Col span={12}>
                          <FormItem label="事发时间" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
                            {getFieldDecorator('incidentDate', {
                              initialValue: moment(incidentDate) || "",
                        ...
                        <Col span={12}>
                          <FormItem label="权力编码" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
                            ...
                            {/* <Option value={1} key={1}>{powerCoding}</Option> */}
                          </FormItem>
                        </Col>
                      </Col>
                      <Col span={24}>
                ...
                <Col span={24} style={{ padding: "0 40px" }}>
                  ...
                  <Col md={8} style={{ marginLeft: '84%' }}>
                    <Button onClick={onClick} style={{ marginRight: '1%' }}>取消</Button>
