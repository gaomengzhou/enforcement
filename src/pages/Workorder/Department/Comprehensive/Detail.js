...
class app extends Component {
  ...
  // 整改前
  renderAttachment() {
    const { uploading, fileList, filePath, previewVisible, previewImage, info, userRole } = this.state;
    const uploadProps = {
      name: 'file',
    ...
    const uploadButton = (
      <div>
        <Button style={{ position: "absolute", top: -6, left: 88 }} className="ant-upload-text" disabled={info.status != '1' && info.status != '2' || userRole == 1}>
          整改前附件
        </Button>
      </div>
    );
  ...
  renderAttachmentTwo() {
    ...
    const uploadButton = (
      <div>
        <Button style={{ position: "absolute", top: -6, left: 108 }} className="ant-upload-text" disabled={(info.status != '5' || userRole != 4 || info.isApplicationClosure != 0) && (info.selfDisposal == 0 || info.status == 9) || userRole == 1}>
          整改后附件
        </Button>
      </div>
    );
  ...
  // 领导决定处置时间
  isManageChange = (i, e) => {
    const { getFieldValue, setFieldsValue, setFields } = this.props.form;
    setFields({ isManage: true })
    console.log(i, e, getFieldValue(`isManage[${i}]`))
  ...
  showModal = type => {
    if (type == 'TG') {
      ...
      this.props.form.validateFields(["assistTime"], (error, values) => {
        if (error) {
          return;
        }
        this.setState(
  ...
  handleOk = type => {
    ...
    if (type == 'TGBH') {
      ...
      values && (reCheckObj.unit = this.state.unitfh);
  ...
  fhUrbanApartmentSelect = (value, key) => {
    this.props.form.resetFields(['fhexecutor']) //fhexecutor
    const { dispatch } = this.props;
    dispatch({
      type: 'compmodels/getUserListByOrgIdAndIsExecutives',
      payload: {
      ...
      callback: res => {
        if (res) {
          if (res.retCode == 1) {
            this.setState({ leaderships: res.list });
          }
        }
      },
    });
  ...
  render() {
    ...
    const SelectAfterfh = props => {
      return (
        <Select onChange={this.getUnitfh} value={info.unit ? info.unit : unitfh} style={{ width: 80 }}>
          <Option value="1" key="1">小时</Option>
          <Option value="2" key="2">天</Option>
        </Select>
      )
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'extend1',
        key: 'extend1',
        render: (text, record) => <a onClick={this.seePhopto.bind(this, record)}>{record.photo && "查看"}</a>,
      },
      {
        title: '下载',
        dataIndex: 'download',
        key: 'download',
        render: (text, record) => <a onClick={this.loopDownPhopto.bind(this, record.photo)}>{record.photo && "下载"}</a>,
      },
      {
        title: '处理描述',
        ...
        <Col span={24}>
          <Col span={this.state.info ? (this.state.info.status >= 5 && this.state.info.dispatchVos[0].executor) ? 5 : 5 : 5}>
            <Form.Item
              ...
              wrapperCol={{ span: 16 }}
              label="处置时限"
              required={this.state.info.status == 5}
              key={index}
            >
              ...
              })(<Input addonAfter={<SelectAfter index={index} />} disabled={(info.dispatchVos && info.dispatchVos.length > 0 && info.dispatchVos[0].isManage == 0 || userRole != 3) && ((flag != 1 && flag != 2) || isManageFlag[index]) || userRole == 1} />)}
            </Form.Item>
          </Col>
          <Col style={{ marginLeft: 40 }} span={this.state.info ? (this.state.info.status >= 5 && this.state.info.dispatchVos[0].executor) ? 6 : 8 : 8}>
            ...
            >
              {getFieldDecorator(`assistDepartment[${index}]`, {
                ...
                <Select
                  placeholder="请选择"
                  disabled={flag != 1 && flag != 2 || userRole == 1}
          ...
          <Col style={{ marginLeft: 40 }} span={this.state.info ? (this.state.info.status >= 5 && this.state.info.dispatchVos[0].executor) ? 5 : 8 : 8}>
            <Form.Item
              labelCol={{ span: 8 }}
              wrapperCol={{ span: 16 }}
              label="处置人员"
            ...
            >
              {getFieldDecorator(`assistWorker[${index}]`, {
                ...
                <Select placeholder="请选择" disabled={flag != 1 && flag != 2 || userRole == 1}>
                  {leadership &&
                    leadership.map(item => {
          ...
          <Col span={24} hidden={(flag != 1 && flag != 2)}>
            <Col span={7}>
              {/* {console.log(info.dispatchVos && info.dispatchVos[index] && info.dispatchVos[index].isManage == "1")} */}
              <Form.Item
                labelCol={{ span: 13 }}
                wrapperCol={{ span: 10 }}
                label="由部门负责人设置"
              ...
              >
                {getFieldDecorator(`isManage[${index}]`, {
                  initialvalue: info.dispatchVos && info.dispatchVos[index] && info.dispatchVos[index].isManage
                })(info.dispatchVos && info.dispatchVos[index] && info.dispatchVos[index].isManage == "1" ?
                  <Checkbox checked onChange={this.isManageChange.bind(this, index)} /> :
                ...
                <Form>
                  <Row>
                            ...
                            <Col span={12}>
                              ...
                              >
                                {getFieldDecorator('ownCommunity', {
                                  ...
                                  <Select onSelect={this.communityGrid} placeholder="请选择" disabled={flag != 1 && flag != 2 || userRole == 1}>
                                    {parentBank &&
                                      parentBank.map(item => {
                                        return (
                          ...
                          <Col span={24}>
                            <Col span={24}>
                              <FormItem label="&nbsp;&nbsp;&nbsp;详细位置" labelCol={{ span: 4 }} wrapperCol={{ span: 20 }}>
                                ...
                                })(<Input disabled={flag != 1 && flag != 2 || userRole == 1} />)}
                              </FormItem>
                            </Col>
                          </Col>
                          <Col span={24}>
                            <Col span={12}>
                              ...
                              >
                                {getFieldDecorator('partiesType', {
                                  initialValue: info.partiesType || "2",
                                  rules: [
                                    {
                                      required: true,
                                      message: '请选择案件小类',
                                  ...
                                  <Select onSelect={this.onSelects} disabled={flag != 1 && flag != 2 || userRole == 1}>
                          ...
                          <Col span={24}>
                            ...
                            <Col span={12}>
                              <FormItem label="权力编码" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
                                ...
                                })(<Select showSearch disabled={flag != 1 && flag != 2 || userRole == 1}>
                          ...
                          <Col span={24}>
                            <Col span={24}>
                              <FormItem label="案件描述" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
                                ...
                                })(<TextArea rows={4} disabled={flag != 1 && flag != 2 || userRole == 1} />)}
                              </FormItem>
                            </Col>
                          </Col>
                    ...
                    <Col span={24} style={{ padding: "0 40px" }}>
                      ...
                      <Col span={24}>
                        <Row hidden={info.isCheck == 0 || userRole != 2}>
                          <h3 style={{ fontWeight: 700, padding: "18px 0 12px 0" }}>复核过程</h3>
                          <Col span={24}>
                            <Col span={5}>
                              <FormItem label="复核时限" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
                                {getFieldDecorator("fhassistTime", {
                                  ...
                                  <Input addonAfter={<SelectAfterfh />} disabled={flag != 1 && flag != 2} />
                                )}
                              </FormItem>
                            </Col>
            ...
            <Row>
              <Col span={24}>
                <FormItem label="审核意见" labelCol={{ span: 4 }} wrapperCol={{ span: 17 }}>
                  {getFieldDecorator('auditOpinions', { initialValue: "审核通过" })(<TextArea rows={4} />)}
                </FormItem>
              </Col>
            </Row>
          </Form>
