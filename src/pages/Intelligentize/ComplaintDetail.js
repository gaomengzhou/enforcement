...
import ComplanintDetailForm from './ComplanintDetailForm';
...
class app extends PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      filePathTwo: '',
      appealTypess: [], // 诉求类型
      majorEmergencyMatterss: [], // 紧急重大类事项
      appealSources: [], // 诉求来源
      returnTypes: [], // 回访类型
      gender: [], // 性别
      parentBank: [], // 所属分行（社区）
  ...
  getSelect(groupCode) {
    ...
    dispatch({
      ...
      callback: res => {
        if (res) {
          groupCode == 'appealTypess' && this.setState({ appealTypess: res });
          groupCode == 'majorEmergencyMatterss' && this.setState({ majorEmergencyMatterss: res });
          groupCode == 'appealSources' && this.setState({ appealSources: res });
          groupCode == 'returnTypes' && this.setState({ returnTypes: res });
  ...
  componentDidMount() {
    ...
    if (this.props[0].preAttachmentIds) {
      const arr = [];
      const arrTwo = [];
      if (this.props[0].preAttachmentIds.length > 0) {
        for (let i = 0; i < this.props[0].preAttachmentIds.length; i += 1) {
          arr.push({
            uid: this.props[0].preAttachmentIds[i].id,
            name: this.props[0].preAttachmentIds[i].fileName,
            status: 'done',
            url: `${picIp}${this.props[0].preAttachmentIds[i].filePath}`,
          });
          arrTwo.push(this.props[0].preAttachmentIds[i].id);
        }
        this.setState({ fileList: arr, filePath: arrTwo });
      }
    }

    if (this.props[0].proAttachmentIds && this.props[0].proAttachmentIds.length > 0) {
      const arr = [];
      const arrTwo = [];
      this.props[0].proAttachmentIds.map(item => {
        arr.push({
          uid: item.id,
          name: item.fileName,
          status: 'done',
    ...
    //           {
    //             info: res.list[0],
    //             dispatchVoss: res.list[0].dispatchVoss
    //               ? res.list[0].dispatchVoss.length > 0
    //                 ? res.list[0].dispatchVoss
    //                 : ['1']
    //               : ['1'],
    ...
    //           },
    //           () => {
    //             const { info, userRole, dispatchVoss, fileArr, fileArrTwo } = this.state;
    //             if (userRole == '4') {
    //               if (info.status != '1') {
    //                 this.getDetailListCourse(dispatchVoss[0].assistId, userRole);
    //               }
    //             } else {
    ...
    this.getSelect('appealTypess');
    this.getSelect('majorEmergencyMatterss');
    this.getSelect('appealSources');
    this.getSelect('returnTypes');
  ...
  systemDispatch = form => {
    ...
    validateFields((error, values) => {
      ...
      values.dispatchVoss = dispatchVoss;
  ...
  showModal = type => {
    if (type == 'TG') {
      ...
      this.setState(
        ...
        () => {
          const { dispatchVoss } = this.state;
          const { dispatch } = this.props;
          dispatch({
            type: 'compmodels/getUserListByOrgIdAndIsExecutive',
            payload: {
              id: dispatchVoss[0].assistDepartment,
            },
            callback: res => {
              if (res) {
                if (res.retCode == 1) {
  ...
  handleOk = type => {
    ...
    const { dispatchVoss, title, info, filePathTwo } = this.state;
    const values = getFieldsValue();
    const obj = dispatchVoss[0];
    const infos = info;
  ...
  buttons = (status, userRole, isCheck, startIsReview, endIsReview) => {
    const { info, dispatchVoss } = this.state;
      ...
      return (
        <Col span={21}>
          <Button
            type="primary"
            style={{ marginRight: 20 }}
            hidden={dispatchVoss && dispatchVoss[0].examine == '1' || info.isReview == 1}
    ...
    return (
      <div id="listTitleContent">
        <div id="listTitleDetailTabEdit">
          <Form>
            <ComplanintDetailForm {...DetailForms} />
          </Form>
        </div>
        <Modal
          title={title}
                ...
                <Col span={12}>
                  <FormItem label="处置时限" labelCol={{ span: 8 }} wrapperCol={{ span: 15 }}>
                    {getFieldDecorator("assistTime", {
                      ...
                      initialValue: (info.dispatchVoss && info.dispatchVoss[0]) ? info.dispatchVoss[0].assistTime : "",
                    })(
                      <Input
                        placeholder="请输入"
                        addonAfter={selectAfter}
            ...
            <Row>
              <Col span={24}>
                <Col span={12}>
                  <FormItem label="处置时限" labelCol={{ span: 8 }} wrapperCol={{ span: 15 }}>
                    {getFieldDecorator("assistTime", {
                      initialValue: (info.postponementVos && info.dispatchVoss[0]) ? info.postponementVos[0].assistTime : "",
                    })(
                      <Input
                        placeholder="请输入"
                        addonAfter={selectAfter}
                ...
                <Col span={12}>
                  <FormItem label="延时时间" labelCol={{ span: 9 }} wrapperCol={{ span: 15 }}>
                    {getFieldDecorator("delayDate", {
                      initialValue: (info.postponementVos && info.dispatchVoss[0]) ? info.postponementVos[0].delayDate : "",
                    })(
                      <Input
                        placeholder="请输入"
                        addonAfter={selectAfter}
              ...
              <Col span={24}>
                <FormItem label="申请说明" labelCol={{ span: 4 }} wrapperCol={{ span: 19 }}>
                  {getFieldDecorator('applicationNote', {
                    initialValue: (info.postponementVos && info.dispatchVoss[0]) ? info.postponementVos[0].applicationNote : "",
                  })(<TextArea disabled />)}
                </FormItem>
              </Col>
              <Col span={24}>
                <FormItem label="申请人" labelCol={{ span: 4 }} wrapperCol={{ span: 19 }}>
                  {getFieldDecorator('applicationPeople', {
                    initialValue: (info.postponementVos && info.dispatchVoss[0]) ? info.postponementVos[0].applicationUserIdDesc : "",
                  })(<Input disabled />)}
                </FormItem>
              </Col>
              <Col span={24}>
