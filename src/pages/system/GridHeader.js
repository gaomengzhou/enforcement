...
class app extends Component {
  ...
  renderTreeNodes = data =>
    data.map(item => {
      if (item.children) {
        return (
          <TreeNode title={item.orgName} key={item.id} value={item.id}>
  ...
  render() {
    const {
      ...
      detail, deleteList, gridSelects,treeValues,onChangeTree
    } = this.props;
          ...
          <Col span={12}>
            ...
            >
              {getFieldDecorator(`ownGridUsers[${index}]`, {
                ...
                >
                  {this.renderTreeNodes(userSource)}
                </TreeSelect>
              )}
            </Form.Item>
          </Col>
        ...
        <div id="tableForm">
          ...
          <div layout="inline">
            <Form layout="inline">
              <Row>
                <Col span={24}>
                  <Col md={8} sm={8} style={{ paddingRight: 40 }}>
                    <FormItem label="网格名称">
                      ...
                      {getFieldDecorator('ownCommunity')(
                        <Select placeholder="请选择">
                          {parentBank &&
                            parentBank.map(item => {
                              return (
                                <Option value={item.desp} key={item.code}>
